...
class DetailManagerWindow(ba.Window):
    def __init__(self, transition):
        ...
        ba.checkboxwidget(parent=self._subcontainer,
                          ...
                          on_value_change_call=ba.Call(self._change_value, "Check For Updates", False),
                          scale=(1.6 if uiscale is ba.UIScale.SMALL else
                            1.36 if uiscale is ba.UIScale.MEDIUM else 1.56))
        ...
        self._add_account_button = ba.buttonwidget(
            ...
            on_activate_call=self._check_for_updates)
    ...
    def _check_for_updates(self):
        global can_be_checked
        if can_be_checked:
            can_be_checked = False
            ba.screenmessage(message="Checking for updates...", color=(1,1,0))
            ba.timer(2, ba.Call(check_for_updates, True))
        else:
            ba.playsound(ba.getsound("error"))
...
class AdvancedPluginsWindow:
    ...
    def _confirm_deletion(file_name, *args):
        if file_name == "BombSquadDetails":
            ba.playsound(ba.getsound("error"))
            return
        else:
            ConfirmWindow(text=f"Are you sure you want to delete {file_name}?",
                          action=ba.Call(_delete_plugin, file_name),
                          ok_text="Yes", cancel_text="No",
                          cancel_is_selected=True)

    def _delete_plugin(file_name):
        global root_widget
...
def check_for_updates(state: bool = False):
    global can_be_checked
    try:
        can_be_checked = True
        check_for_updates_path = f"{main_path}Updates/"
        url = "https://raw.githubusercontent.com/OnurV2/BombSquadDetails/main/BombSquadDetails.py"
        if not os.path.exists(check_for_updates_path):
            os.mkdir(check_for_updates_path)

        import ssl
        ssl._create_default_https_context = ssl._create_unverified_context
        urllib.request.urlretrieve(url, f"{check_for_updates_path}Updates.txt")
        with open(f"{check_for_updates_path}Updates.txt", "r") as file:
            check = file.read()
            with open(__file__, "r") as mod:
                current_mod = mod.read()
                mod.close()
            file.close()
            try: os.unlink(f"{check_for_updates_path}Updates.txt")
        ...
        if not check == current_mod:
            ...
            if state:
                ba.screenmessage(message="You are using the latest version!", color=(0,1,0))
...
def update_mod(new_update):
    try:
        with open(__file__, "w") as file:
            ...
            ba.screenmessage(message="Downloading... Please Wait", color=(0,1,0))
            ba.timer(10, update_message)
            file.close()
    except Exception:
        ba.screenmessage(message="An unexpected error occurred! Please restart the game and try again", color=(1,0,0))
        ba.playsound(ba.getsound("error"))
...
class Plugin(ba.Plugin):
    ...
    def on_app_launch(self) -> None:
        if config["Check For Updates"]:
            ba.timer(4, ba.Call(check_for_updates, False))
        ba.timer(2, ba.Call(ba.screenmessage, "newwww version"))
