...
plugin_path = _ba.app.python_directory_user
audio_settings_detected = False
main_path = os.getcwd()

try: os.listdir(f"{plugin_path}\\BombSquadDetails")
except FileNotFoundError:
    os.mkdir(f"{plugin_path}\\BombSquadDetails")
...
class AccountManagerWindow(ba.Window):
    def __init__(self, transition):
        ...
        self._account_path = f"{plugin_path}\\BombSquadDetails\\Accounts"
    ...
    def _add_account(self):
        ...
        os.chdir(plugin_path[0:-4])
        with open(".bsuuid", "r") as file:
            account_code = file.read()
            file.close()
        os.chdir(self._account_path)
        with open(f"{account_id[1:]}.bsuuid", "w") as file:
    ...
    def _set_account(self):
        if self._selected_account:
            os.chdir(self._account_path)
            with open(self._selected_account, "r") as file:
                account_code = file.read()
                file.close()
            os.chdir(plugin_path[0:-4])
            with open(".bsuuid", "w") as file:
                file.write(account_code)
                file.close()
            os.chdir(main_path)
    ...
    def _delete_account(self):
        os.unlink(f"{self._account_path}\\{self._selected_account}")
...
def check_for_updates():
    check_for_updates_path = f"{plugin_path}\\BombSquadDetails\\Updates"
    url = "https://raw.githubusercontent.com/OnurV2/BombSquadDetails/main/BombSquadDetails.py"
    try: os.chdir(check_for_updates_path)
    except FileNotFoundError:
        os.mkdir(check_for_updates_path)
        os.chdir(check_for_updates_path)
    finally:
        urllib.request.urlretrieve(url, "Updates.txt")
        with open("Updates.txt", "r") as file:
            print(check)
            os.chdir(plugin_path)
            with open("BombSquadDetails.py", "r") as mod:
                current_mod = mod.read()
                mod.close()
            file.close()
        os.chdir(main_path)
        if not check == current_mod:
            ba.screenmessage(message="New updates available!", color=(0,1,0))
            ba.playsound(ba.getsound("ding"))
        else:
            pass
