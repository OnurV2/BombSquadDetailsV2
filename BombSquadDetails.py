...
default_config = {'Leave The Server Directly': False,
                  ...
                  'Mute Chat': ba.app.config.resolve('Chat Muted')}
...
class ChatManagerWindow(ba.Window):
    def __init__(self, transition: str, close_directly: bool):
        ...
        ba.checkboxwidget(parent=self._subcontainer,
                          ...
                          on_value_change_call=ba.Call(self._change_value, 'Reconnect Server Directly', False),
                          scale=main_scale)
...
class NewPartyWindow:
    global confirm_copy_message, copy_message, confirm_open_url, vote_kick_player, confirm_reconnect_server, reconnect_server
    ...
    def _new_popup_menu_selected_choice(self, popup_window: PopupMenuWindow,
        ...
        if self._popup_type == 'partyMemberPress':
            ...
            message_status_text = 'show in-game messages' if config['Hide Chat Messages'] else 'hide in-game messages'
            if choice in ('mute', 'unmute'):
                ...
                config['Hide Chat Messages'] = False if config['Hide Chat Messages'] else True
    ...
    def _new_close(self):
        self._old_close()
        message_status = config['Hide Chat Messages']
