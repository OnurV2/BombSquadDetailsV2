...
class ChatManagerWindow(ba.Window):
    def __init__(self, transition: str, close_directly: bool):
        ...
        ba.checkboxwidget(parent=self._subcontainer,
                          ...
                          scale=main_scale)
        ba.checkboxwidget(parent=self._subcontainer,
                          text='Remove chat limit',
                          size=(500, 47),
                          value=config['Remove Chat Limit'],
                          on_value_change_call=ba.Call(self._change_value, 'Remove Chat Limit', False),
                          scale=main_scale)
        ba.checkboxwidget(parent=self._subcontainer,
...
chat_messages = []
append_msg = True
def _new_get_chat_messages():
    global chat_messages
    return chat_messages
_ba.get_chat_messages = _new_get_chat_messages
                ...
                if len(chat_messages) > 40:
                    for i in chat_messages:
                        if len(chat_messages) > 40:
                            ...
                            break
        if not config['Remove Chat Limit']:
